...
## Correccion aplicada (error metadata-generation-failed)

Ese error en Render ocurre normalmente porque pip intenta compilar un
paquete (por ejemplo pandas) al no encontrar una version precompilada para
la version de Python que usa Render por defecto (suele ser muy reciente).

Se corrigio asi:
1. Se elimino "pandas" de requirements.txt (no lo usa la app web).
2. Se agrego runtime.txt fijando Python 3.11.9, version estable y compatible
   con Flask, gunicorn y openpyxl sin necesidad de compilar nada.
...
2. Sube esta carpeta completa a un repositorio de GitHub (puede ser privado),
   asegurandote de que app.py, requirements.txt, runtime.txt y Procfile
   queden en la RAIZ del repositorio (no dentro de una subcarpeta extra).
3. En Render: "New +" -> "Web Service" -> conecta tu repositorio.
4. Render detectara automaticamente render.yaml y el Procfile.
   Si te pide configurarlo manualmente:
   - Build Command: pip install -r requirements.txt
   - Start Command: gunicorn app:app
5. Da clic en "Create Web Service". En unos minutos te entrega una URL
   publica (ej. https://checador-corporativo.onrender.com).
...
1. Instala Python 3.11 (recomendado, para igualar el entorno de Render).
